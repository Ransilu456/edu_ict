import template from './course-view.html?raw';
...
class CourseView extends HTMLElement {
  ...
  initCourse() {
    ...
    window.addEventListener('resize', () => {
      this.drawCourseWires();
    });

    // Expose functions globally
    window.drawCourseWires = () => this.drawCourseWires();
    ...
    // Attach footer CTA click event
    const ctaBtn = this.querySelector('#cta-btn');
    if (ctaBtn) {
      ctaBtn.onclick = () => this.handleCtaClick();
    }
  }

  initLessonSelect() {
    const lessonSelect = this.querySelector('#lesson-select');
    if (lessonSelect) {
  ...
  renderLesson() {
    ...
    const track = this.querySelector('#course-progress');
    if (track) {
      track.innerHTML = '';
      lessons.forEach((l, idx) => {
    ...
    this.querySelector('#lesson-title').innerHTML = lesson.title;
    this.querySelector('#lesson-text').innerHTML = lesson.text;
    
    // Render Quiz Choices
    const quizContainer = this.querySelector('#quiz-options');
    ...
    this.renderVisualSimulation(lesson.type);
  }
  ...
  selectQuizOption(idx) {
    ...
    this.querySelector('#cta-btn').disabled = false;
  }
  ...
  handleCtaClick() {
    ...
    if (!quizSubmitted) {
      ...
      // Disable inputs
      this.querySelectorAll('.quiz-option').forEach((opt, index) => {
      ...
      if (isCorrect) {
        ...
        // Mark step progress
        const steps = this.querySelectorAll('.progress-step');
  ...
  renderVisualSimulation(type) {
    const rightPane = this.querySelector('#course-sim-container');
    if (!rightPane) return;
    rightPane.innerHTML = '';
    ...
    if (type === 'binary-bulb') {
      ...
      const bulbSwitch = this.querySelector('#bulb-switch');
      const bulbGraphic = this.querySelector('#bulb-graphic');
      const voltageLabel = this.querySelector('#bulb-voltage-label');
      ...
      const baseNode = this.querySelector('#trans-base');
      const outNode = this.querySelector('#trans-path-out');
      const container = this.querySelector('.transistor-component');
      const stateText = this.querySelector('#trans-state-text');
      const cLine = this.querySelector('#collector-line');
      const bLine = this.querySelector('#base-line');
      const eLine = this.querySelector('#emitter-line');
      const gateLine = this.querySelector('#npn-gate');
      ...
      const inputs = this.querySelectorAll('.course-input');
      inputs.forEach(input => {
        input.addEventListener('click', () => {
          if (window.playSound) window.playSound('toggle');
          const isActive = input.classList.toggle('active');
          input.innerText = isActive ? '1' : '0';
          this.evaluateBasicGate(type);
        });
      });
      
      this.evaluateBasicGate(type);
      setTimeout(() => this.drawCourseWires(), 30);
      ...
      html = `
        <div style="text-align: center; max-width: 400px; padding: 2rem;">
          ...
          <button class="btn-primary" style="margin: 0 auto; width: auto;" id="course-complete-go-sandbox">
            Open Sandbox Canvas
          </button>
        </div>
      `;
      ...
      // Fix redirections to switch tabs programmatically
      const btn = this.querySelector('#course-complete-go-sandbox');
      if (btn) {
        btn.onclick = () => {
          document.querySelector('[data-target="sandbox-view"]')?.click();
        };
      }
    }
  }
  ...
  evaluateBasicGate(type) {
    const inA = this.querySelector('#node-in-a')?.classList.contains('active') ? 1 : 0;
    const inB = this.querySelector('#node-in-b')?.classList.contains('active') ? 1 : 0;
    const outNode = this.querySelector('#node-out-y');
    const gateBadge = this.querySelector('#course-gate-badge');
  ...
  evaluateHalfAdder() {
    ...
    const sumNode = this.querySelector('#node-out-sum');
    const carryNode = this.querySelector('#node-out-carry');
    const gXor = this.querySelector('#adder-gate-xor');
    const gAnd = this.querySelector('#adder-gate-and');
  ...
  evaluateFullAdder() {
    const inA = this.querySelector('#node-in-a').classList.contains('active') ? 1 : 0;
    const inB = this.querySelector('#node-in-b').classList.contains('active') ? 1 : 0;
    const inCin = this.querySelector('#node-in-cin').classList.contains('active') ? 1 : 0;
    ...
    const sumNode = this.querySelector('#node-out-sum');
    const coutNode = this.querySelector('#node-out-cout');
    
    const faXor1 = this.querySelector('#fa-xor1');
    const faAnd1 = this.querySelector('#fa-and1');
    const faXor2 = this.querySelector('#fa-xor2');
    const faAnd2 = this.querySelector('#fa-and2');
    const faOr = this.querySelector('#fa-or');
  ...
  drawCourseWires() {
    const svg = this.querySelector('#course-wires-svg');
    if (!svg) return;
    ...
    if (type === 'gate-not' || type === 'gate-and' || type === 'gate-or' || type === 'gate-xor') {
      ...
      const nodeA = this.querySelector('#node-in-a');
      const nodeB = this.querySelector('#node-in-b');
      const xorGate = this.querySelector('#adder-gate-xor');
      const andGate = this.querySelector('#adder-gate-and');
      const sumNode = this.querySelector('#node-out-sum');
      const carryNode = this.querySelector('#node-out-carry');
      ...
      this.createBezierWire(svg, nodeA, xorGate, isAActive, 0.4, 0.25);
      this.createBezierWire(svg, nodeA, andGate, isAActive, 0.4, 0.25);
      this.createBezierWire(svg, nodeB, xorGate, isBActive, 0.6, 0.75);
      this.createBezierWire(svg, nodeB, andGate, isBActive, 0.6, 0.75);
      this.createBezierWire(svg, xorGate, sumNode, sumNode.classList.contains('active'));
      this.createBezierWire(svg, andGate, carryNode, carryNode.classList.contains('active'));
      
    } else if (type === 'full-adder') {
      const nodeA = this.querySelector('#node-in-a');
      const nodeB = this.querySelector('#node-in-b');
      const nodeCin = this.querySelector('#node-in-cin');
      
      const xor1 = this.querySelector('#fa-xor1');
      const and1 = this.querySelector('#fa-and1');
      const xor2 = this.querySelector('#fa-xor2');
      const and2 = this.querySelector('#fa-and2');
      const orGate = this.querySelector('#fa-or');
      ...
      this.createBezierWire(svg, nodeA, xor1, isAActive, 0.35, 0.2);
      this.createBezierWire(svg, nodeA, and1, isAActive, 0.35, 0.2);
      this.createBezierWire(svg, nodeB, xor1, isBActive, 0.5, 0.8);
      this.createBezierWire(svg, nodeB, and1, isBActive, 0.5, 0.8);
      
      const isXor1Active = isAActive !== isBActive;
      
      this.createBezierWire(svg, xor1, xor2, isXor1Active, 0.5, 0.2);
      this.createBezierWire(svg, xor1, and2, isXor1Active, 0.5, 0.2);
      this.createBezierWire(svg, nodeCin, xor2, isCinActive, 0.65, 0.8);
      this.createBezierWire(svg, nodeCin, and2, isCinActive, 0.65, 0.8);
      
      const isAnd1Active = isAActive && isBActive;
      const isAnd2Active = isXor1Active && isCinActive;
      
      this.createBezierWire(svg, and1, orGate, isAnd1Active, 0.5, 0.2);
      this.createBezierWire(svg, and2, orGate, isAnd2Active, 0.5, 0.8);
      this.createBezierWire(svg, xor2, sumNode, sumNode.classList.contains('active'));
      this.createBezierWire(svg, orGate, coutNode, coutNode.classList.contains('active'));
    }
  }
